...
NJMCDecoder::decodeInstruction(ADDRESS pc, int delta, UserProc *proc /* = NULL */)
{
	...
	// Callee prologues
	//
	else if ((logue = InstructionPatterns::gcc_frame(csr, hostPC, locals)) != NULL) {
		/*
	...
	} else if ((logue = InstructionPatterns::gcc_frameless(csr, hostPC, locals)) != NULL) {
		/*
	...
	} else if ((locals = 8, logue = InstructionPatterns::param_reloc1(csr, hostPC, libstub, locals)) != NULL) {
		/*
	...
	//
	...
	} else if ((logue = InstructionPatterns::gcc_unframeless1(csr, hostPC)) != NULL) {
		/*
	...
	} else if ((logue = InstructionPatterns::gcc_unframeless2(csr, hostPC)) != NULL) {
		/*
		 * Removal of current frame for gcc (where no frame pointer was used)
		 */
		result.numBytes = hostPC - saveHostPC;
		...
		SHOW_ASM("bare_ret");
	} else if ((logue = InstructionPatterns::bare_ret_anulled(csr, hostPC)) != NULL) {
		/*
...
void
NJMCDecoder::low_level(list<RT *> *&RTs, ADDRESS hostPC, ADDRESS pc, DecodeResult &result, ADDRESS &nextPC)
{
...
SemStr *
NJMCDecoder::dis_xd(ADDRESS hostpc)
{
	SemStr *result;
	match hostpc to
	| x_addr_nots(x) => {
...
SemStr *
NJMCDecoder::dis_c_wcr(unsigned hostPC)
{
	return 0;
}

SemStr *
NJMCDecoder::dis_ct(unsigned hostPC)
{
	return 0;
}

SemStr *
NJMCDecoder::dis_Freg(int regNum, int fmt)
{
