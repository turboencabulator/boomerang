...
#if 0
void
NJMCDecoder::Cmplt_1(ADDRESS hostpc1)
{
}

void
NJMCDecoder::Cmplt_2(ADDRESS hostpc2)
{
}

void
NJMCDecoder::Cmplt_3(ADDRESS hostpc3)
{
}
#endif
...
void
NJMCDecoder::dis_c_c(ADDRESS hostpc)
{
	static char *logw[] = {
		"", ",=", ",<", ",OD",
		",TR", ",!=", ",>=", ",EV"
...
void
NJMCDecoder::dis_addr(ADDRESS hostpc)
{
	match hostpc to
	| index_addr(x, ss, b, cmplt) => {
		astr += sprintf(astr, "%s(%s,%s)", b_06_names[x], s2_16_names[ss], b_06_names[b]);
		dis_c_addr(cmplt);
	...
	| sdispl_addr(im5, ss, b, cmplt) => {
		astr += sprintf(astr, "%d(%s,%s)", im5, s2_16_names[ss], b_06_names[b]);
		dis_c_addr(cmplt);
	...
	| ldispa12_addr(ldisp, ss, b, cmplt) => {
		astr += sprintf(astr, "%d(%s,%s)", ldisp, s2_16_names[ss], b_06_names[b]);
		dis_c_addr(cmplt);
	...
	| bea17_addr(offset, ss, b) => {
		astr += sprintf(astr, "%d(%s,%s)", offset, s3_16_names[ss], b_06_names[b]);
...
unsigned long
NJMCDecoder::dis_c_wcr(ADDRESS hostpc)
{
	unsigned long regl;
	match hostpc to
	| c_mfctl(r_06) => {
...
void
NJMCDecoder::dis_flt_fmt(int fmt)
{
...
int
NJMCDecoder::decodeAssemblyInstruction(ADDRESS pc, int delta)
{
	char sCmplt[32];
	unsigned long r1, r2;
	sCmplt[0] = '\0';
	...
	| LDSID(s2_16, b_06, t_27) [name] => {
		astr += sprintf(astr, "%s  (%s,%s),%s", name, s2_16_names[s2_16], b_06_names[b_06], t_27_names[t_27]);
		strcat(constrName, "LDSID ");
	}
	| MTSP(r_11, sr) [name] => {
		astr += sprintf(astr, "%s  %s,%s", name, r_11_names[r_11], s3_16_names[sr]);
		strcat(constrName, "MTSP ");
	}
	| MFSP(sr, t_27) [name] => {
		astr += sprintf(astr, "%s  %s,%s", name, s3_16_names[sr], t_27_names[t_27]);
	...
	| fstores(c_faddr, r, faddr) [name] => {
		...
		astr += sprintf(astr, "unrecog. %02X %08X", (getDword(hostPC) >> 26) & 0x3F, getDword(hostPC) & 0x03FFFFFF);
	endmatch
