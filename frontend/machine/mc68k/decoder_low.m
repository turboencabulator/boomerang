 ...
 *============================================================================*/
RT *
assignTemp(SemStr *src, int size, SemStr *&temp)
{
 ...
 *============================================================================*/
RT *
sgnExTemp(SemStr *src, int size, int size2, SemStr *&temp)
{
	if (temp1 == 0) temp1 = theSemTable.addItem("temp1");
	// Lhs: r[temp1]
...
NJMCDecoder::decodeLowLevelInstruction(ADDRESS hostPC, ADDRESS pc, DecodeResult &result)
{
	...
	| _immEAb(i8, ea) [name] =>
		...
		int bump = 0, bumpr;
		chop2ndLast(name);
		RTs = instantiate(pc, sslName, DIS_I8, daEA(ea, pc, bump, bumpr, 8));
		ADDBUMP;
		result.numBytes += 2;
	...
	| _immEAb^".ex"(i8, eax, x) [name] =>
		chopBoth(name);
		RTs = instantiate(pc, sslName, DIS_I8, daEAX(eax, x, result, pc, 8));
		result.numBytes += 2;

	| _immEAw(i16, ea) [name] =>
		int bump = 0, bumpr;
		chop2ndLast(name);
		RTs = instantiate(pc, sslName, DIS_I16, daEA(ea, pc, bump, bumpr, 16));
		ADDBUMP;
		result.numBytes += 2;
	...
	| _immEAw^".ex"(i16, eax, x) [name] =>
		chopBoth(name);
		RTs = instantiate(pc, sslName, DIS_I16, daEAX(eax, x, result, pc, 16));
		result.numBytes += 2;

	| _immEAl(i32, ea) [name] =>
		int bump = 0, bumpr;
		chop2ndLast(name);
		RTs = instantiate(pc, sslName, DIS_I32, daEA(ea, pc, bump, bumpr, 32));
		ADDBUMP;
		result.numBytes += 4;

	| _immEAl^".ex"(i32, eax, x) [name] =>
		chopBoth(name);
		RTs = instantiate(pc, sslName, DIS_I32, daEAX(eax, x, result, pc, 32));
		result.numBytes += 4;
	...
	| _bits(n, ea) [name] =>
		...
		// or 32 for registers
		int bump = 0, bumpr;
		RTs = instantiate(pc, name, DIS_DN(32), daEA(ea, pc, bump, bumpr, 8));
		ADDBUMP;

	| _bits^".ex"(n, eax, x) [name] =>
		chopDotex(name);
		RTs = instantiate(pc, sslName, DIS_DN(32), daEAX(eax, x, result, pc, 8));
	...
	| btst^".ex"(n, eax, x) =>
		RTs = instantiate(pc, "btst", DIS_DN(32), dBEAX(eax, x, result, pc, delta, 8));
	...
	| btsti.ex(i8, eax, x) =>
		RTs = instantiate(pc, "btst", DIS_I8, dBEAX(eax, x, result, pc, delta, 8));
		result.numBytes += 2;
	...
	| _move^".ex"(eax, x, ea2) [name] =>
		...
		strcpy(sslName, name);
		sslName[5] = '\0';  // truncate name
		RTs = instantiate(pc, sslName, msEAX(eax, x, result, pc, delta, siz), mdEA(ea2, pc, bump, bumpr, siz));
		ADDBUMP;
	...
	| _move^".exl"(eaxl, d32, ea2) [name] =>
		...
		strcpy(sslName, name);
		sslName[5] = '\0';  // truncate name
		RTs = instantiate(pc, sslName, msEAXL(eaxl, d32, result, pc, siz), mdEA(ea2, pc, bump, bumpr, siz));
		ADDBUMP;
	...
	| _move^".mx"(ea, eax2, x2) [name] =>
		...
		strcpy(sslName, name);
		sslName[5] = '\0';  // truncate name
		RTs = instantiate(pc, sslName, msEA(ea, pc, bump, bumpr, siz), mdEAX(eax2, x2, result, pc, siz));
		ADDBUMP;
	...
	| _move^".emx"(eax, x, eax2, x2) [name] =>
		...
		strcpy(sslName, name);
		sslName[5] = '\0';  // truncate name
		RTs = instantiate(pc, sslName, msEAX(eax, x, result, pc, delta, siz), mdEAX(eax2, x2, result, pc, siz));
	...
	| _move^".emxl"(eaxl, d32, eax2, x2) [name] =>
		...
		strcpy(sslName, name);
		sslName[5] = '\0';  // truncate name
		RTs = instantiate(pc, sslName, msEAXL(eaxl, d32, result, pc, siz), mdEAX(eax2, x2, result, pc, siz));
	...
	| _oneEAdaL^".ex"(eax, x) [name] =>
		chopDotex(name);
		RTs = instantiate(pc, sslName, daEAX(eax, x, result, pc, 32));
	...
	| _oneEAc^".ex"(eax, x) [name] =>
		strcpy(sslName, name);
		sslName[3] = '\0';
		RTs = instantiate(pc, sslName, cEAX(eax, x, result, pc, delta, 32));
	...
	| moveToCCR.ex(eax, x) [name] =>
		RTs = instantiate(pc, name, dWEAX(eax, x, result, pc, delta, 16));
	...
	| movermw(i16, ea) =>
		...
		int bump = 0, bumpr;
		RTs = instantiate(pc, "storem.w", DIS_I16, rmEA(ea, pc, bump, bumpr, 16));
		result.numBytes += 2;
		ADDBUMP;

	| movermw.ex(i16, eax, x) =>
		RTs = instantiate(pc, "storem.w", DIS_I16, rmEAX(eax, x, result, pc, 16));
		result.numBytes += 2;

	| movemrw(ea, i16) =>
		// HACK! Requires work
		int bump = 0, bumpr;
		RTs = instantiate(pc, "loadm.w", DIS_I16, mrEA(ea, pc, bump, bumpr, 16));
		result.numBytes += 2;
		ADDBUMP;

	| movemrw.ex(eax, x, i16) =>
		RTs = instantiate(pc, "loadm.w", DIS_I16, mrEAX(eax, x, result, pc, delta, 16));
		result.numBytes += 2;

	| moverml(i16, ea) =>
		// HACK! Needs work
		int bump = 0, bumpr;
		RTs = instantiate(pc, "storem.l", DIS_I16, rmEA(ea, pc, bump, bumpr, 32));
		result.numBytes += 2;
		ADDBUMP;

	| moverml.ex(i16, eax, x) =>
		RTs = instantiate(pc, "storem.l", DIS_I16, rmEAX(eax, x, result, pc, 32));
		result.numBytes += 2;

	| movemrl(ea, i16) =>
		// HACK! Requires work
		int bump = 0, bumpr;
		RTs = instantiate(pc, "loadm.l", DIS_I16, mrEA(ea, pc, bump, bumpr, 32));
		result.numBytes += 2;
		ADDBUMP;

	| movemrl.ex(eax, x, i16) =>
		RTs = instantiate(pc, "loadm.l", DIS_I16, mrEAX(eax, x, result, pc, delta, 32));
		result.numBytes += 2;
	...
	| lea.ex(eax, x, n) =>
		RTs = instantiate(pc, "lea", cEAX(eax, x, result, pc, delta, 32), DIS_AN);
	...
	| _aluqW^".ex"(i8, eax, x) [name] =>
		chopBoth(name);             // addqb.ex -> addb
		if (i8 == 0) i8 = 8;        // Quirk of the addq/subq instr
		RTs = instantiate(pc, sslName, DIS_I8, alEAX(eax, x, result, pc, 16));
	...
	| _alurdw^".ex"(eax, x, n) [name] =>
		RTs = instantiate(pc, name, dWEAX(eax, x, result, pc, delta, 16), DIS_DN(16));
	...
	| _alurdB^".ex"(eax, x, n) [name] =>
		chopBoth(name);
		RTs = instantiate(pc, sslName, dEAX(eax, x, result, pc, delta, 8), DIS_DN(8));

	| _alurdW(ea, n) [name] =>
		int bump = 0, bumpr;
		chop2ndLast(name);
		RTs = instantiate(pc, sslName, dEA(ea, pc, bump, bumpr, 16), DIS_DN(16));
		ADDBUMP;

	| _alurdW^".ex"(eax, x, n) [name] =>
		chopBoth(name);
		RTs = instantiate(pc, sslName, dEAX(eax, x, result, pc, delta, 16), DIS_DN(16));
	...
	| _alurdL^".ex"(eax, x, n) [name] =>
		chopBoth(name);
		RTs = instantiate(pc, sslName, dEAX(eax, x, result, pc, delta, 32), DIS_DN(32));
	...
	| _alumB^".ex"(n, eax, x) [name] =>
		chopBoth(name);
		RTs = instantiate(pc, sslName, DIS_DN(8), maEAX(eax, x, result, pc, 8));

	| _alumW(n, ea) [name] =>
		int bump = 0, bumpr;
		chop2ndLast(name);
		RTs = instantiate(pc, sslName, DIS_DN(16), maEA(ea, pc, bump, bumpr, 16));
		ADDBUMP;

	| _alumW^".ex"(n, eax, x) [name] =>
		chopBoth(name);
		RTs = instantiate(pc, sslName, DIS_DN(16), maEAX(eax, x, result, pc, 16));

	| _alumL(n, ea) [name] =>
		int bump = 0, bumpr;
		chop2ndLast(name);
		RTs = instantiate(pc, sslName, DIS_DN(32), maEA(ea, pc, bump, bumpr, 32));
		ADDBUMP;

	| _alumL^".ex"(n, eax, x) [name] =>
		chopBoth(name);
		RTs = instantiate(pc, sslName, DIS_DN(32), maEAX(eax, x, result, pc, 32));
	...
	| _aluaW^".ex"(eax, x, n) [name] =>
		chopBoth(name);
		RTs = instantiate(pc, sslName, awlEAX(eax, x, result, pc, delta, 16), DIS_AN);
	...
	| _aluaL^".ex"(eax, x, n) [name] =>
		chopBoth(name);
		RTs = instantiate(pc, sslName, awlEAX(eax, x, result, pc, delta, 32), DIS_AN);
	...
	| eorb.ex(n, eax, x) [name] =>
		RTs = instantiate(pc, name, DIS_DN(8), daEAX(eax, x, result, pc, 8));
	...
	| eorw.ex(n, eax, x) [name] =>
		RTs = instantiate(pc, name, DIS_DN(16), daEAX(eax, x, result, pc, 16));
	...
	| eorl.ex(n, eax, x) [name] =>
		RTs = instantiate(pc, name, DIS_DN(32), daEAX(eax, x, result, pc, 32));
	...
	| _alurB^".ex"(eax, x, n) [name] =>
		chopDotex(name);           // Fix the name
		RTs = instantiate(pc, sslName, amEAX(eax, x, result, pc, delta, 8), DIS_DN(8));

	| _alurW(ea, n) [name] =>
		int bump = 0, bumpr;
		chopDotex(name);           // Fix the name
		RTs = instantiate(pc, sslName, amEA(ea, pc, bump, bumpr, 16), DIS_DN(16));
		ADDBUMP;

	| _alurW^".ex"(eax, x, n) [name] =>
		chopDotex(name);           // Fix the name
		RTs = instantiate(pc, sslName, amEAX(eax, x, result, pc, delta, 16), DIS_DN(16));

	| _alurL(ea, n) [name] =>
		int bump = 0, bumpr;
		chopDotex(name);           // Fix the name
		RTs = instantiate(pc, sslName, amEA(ea, pc, bump, bumpr, 32), DIS_DN(32));
		ADDBUMP;

	| _alurL^".ex"(eax, x, n) [name] =>
		chopDotex(name);           // Fix the name
		RTs = instantiate(pc, sslName, amEAX(eax, x, result, pc, delta, 32), DIS_DN(32));
